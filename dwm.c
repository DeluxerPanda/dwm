...
enum { NetSupported, NetSystemTray, NetSystemTrayOP, NetSystemTrayOrientation, NetSystemTrayVisual,
	NetWMName, NetWMState, NetWMFullscreen, NetActiveWindow, NetWMWindowType, NetWMWindowTypeDock,NetWMIcon,
...
buttonpress(XEvent *e)
{
    ...
    // /* Custom action for ClkStatusText */
    // if (click == ClkStatusText && ev->button == Button1) {
    //     system("st -e calcure");  // Open the 'st' terminal on left-click of the status text
    // }
}
...
updatestatus(void)
{
	...
	if (!gettextprop(root, XA_WM_NAME, stext, sizeof(stext)))
		strcpy(stext, "delux dwm-"VERSION);
	for(m = mons; m; m = m->next)
		drawbar(m);
}
