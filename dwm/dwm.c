...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - stw - x) > bh) {
		...
		drw_setscheme(drw, scheme[SchemeURL]);
			char titledisplay[256];
			snprintf(titledisplay, sizeof(titledisplay), "| %.252s", m->sel->name);
			drw_text(drw, x, 0, w, bh, 0, titledisplay, 0);
			if (m->sel->isfloating)
